...
@section('title', (app()->getLocale() == 'ar' ? 'تعديل نشاط المشروع - ' : 'Edit Activity - ') . $project->getTranslation('name', app()->getLocale()))
@section('header', (app()->getLocale() == 'ar' ? 'تعديل نشاط المشروع - ' : 'Edit Activity - ') . $project->getTranslation('name', app()->getLocale()))

@section('content')
<div class="bg-white p-6 md:p-8 rounded-xl shadow-sm border border-gray-200 max-w-7xl mx-auto">
    <div class="flex items-center justify-between mb-6 pb-4 border-b border-gray-100">
        <div>
            <h3 class="text-xl font-bold text-gray-800">{{ app()->getLocale() == 'ar' ? 'تعديل تفاصيل النشاط / التدخل' : 'Edit Activity / Intervention Details' }}</h3>
    ...
    <form action="{{ route('admin.projects.activities.update', [$project->id, $activity->id]) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div id="content-ar" class="lang-content space-y-6">
                    <div class="bg-gray-50 p-5 rounded-lg border border-gray-100 space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="title[ar]" value="{{ old('title.ar', $activity->getTranslation('title', 'ar')) }}" placeholder="مثال: تقديم سلال غذائية للأسر النازحة" class="w-full p-2.5 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white text-right" dir="rtl">
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="description[ar]" rows="4" placeholder="اكتب وصفاً مختصراً للنشاط هنا..." class="w-full p-2.5 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white text-right" dir="rtl">{{ old('description.ar', $activity->getTranslation('description', 'ar')) }}</textarea>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="detail[ar]" rows="3" placeholder="أية تفاصيل إضافية مثل عدد المستفيدين أو طبيعة المواد الموزعة..." class="w-full p-2.5 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white text-right" dir="rtl">{{ old('detail.ar', $activity->getTranslation('detail', 'ar')) }}</textarea>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" name="location[ar]" value="{{ old('location.ar', $activity->getTranslation('location', 'ar')) }}" placeholder="مثال: الفاشر - مراكز الإيواء - شمال دارفور" class="w-full p-2.5 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white text-right" dir="rtl">
                            </div>
                            <div>
                                ...
                                <input type="text" name="date[ar]" value="{{ old('date.ar', $activity->getTranslation('date', 'ar')) }}" placeholder="مثال: مارس - أبريل 2024" class="w-full p-2.5 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white text-right" dir="rtl">
                            </div>
                        </div>

                        <div>
                            <label class="block text-sm font-semibold mb-1 text-gray-600" dir="rtl">الجهة الممولة <span class="text-red-500">*</span></label>
                            <input type="text" name="funded_by[ar]" value="{{ old('funded_by.ar', $activity->getTranslation('funded_by', 'ar')) }}" placeholder="مثال: منظمة بلان سودان" class="w-full p-2.5 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white text-right" dir="rtl">
                        </div>
                    </div>
                </div>
                ...
                <div id="content-en" class="lang-content space-y-6 hidden">
                    <div class="bg-gray-50 p-5 rounded-lg border border-gray-100 space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="title[en]" value="{{ old('title.en', $activity->getTranslation('title', 'en')) }}" placeholder="e.g. Provision of food security materials" class="w-full p-2.5 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white" dir="ltr">
                        </div>

                        <div>
                            <label class="block text-sm font-semibold mb-1 text-gray-600">Activity Description <span class="text-red-500">*</span></label>
                            <textarea name="description[en]" rows="4" placeholder="Write a short description of the activity here..." class="w-full p-2.5 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white" dir="ltr">{{ old('description.en', $activity->getTranslation('description', 'en')) }}</textarea>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="detail[en]" rows="3" placeholder="Any extra information e.g. number of beneficiaries..." class="w-full p-2.5 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white" dir="ltr">{{ old('detail.en', $activity->getTranslation('detail', 'en')) }}</textarea>
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-semibold mb-1 text-gray-600">Location / Center <span class="text-red-500">*</span></label>
                                <input type="text" name="location[en]" value="{{ old('location.en', $activity->getTranslation('location', 'en')) }}" placeholder="e.g. El fasher - Shelters center North Darfur" class="w-full p-2.5 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white" dir="ltr">
                            </div>
                            <div>
                                <label class="block text-sm font-semibold mb-1 text-gray-600">Date / Period <span class="text-red-500">*</span></label>
                                <input type="text" name="date[en]" value="{{ old('date.en', $activity->getTranslation('date', 'en')) }}" placeholder="e.g. March 2024 to April 2024" class="w-full p-2.5 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white" dir="ltr">
                            </div>
                        </div>

                        <div>
                            <label class="block text-sm font-semibold mb-1 text-gray-600">Funded By <span class="text-red-500">*</span></label>
                            <input type="text" name="funded_by[en]" value="{{ old('funded_by.en', $activity->getTranslation('funded_by', 'en')) }}" placeholder="e.g. Plan Sudan" class="w-full p-2.5 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white" dir="ltr">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="lg:col-span-1 space-y-6">
                <div class="bg-gray-50 p-5 rounded-lg border border-gray-100 space-y-4">
                    ...
                    <!-- Status -->
                    @php
                        $currentStatus = strtolower(str_replace(' ', '_', $activity->getTranslation('status', 'en') ?? 'completed'));
                    @endphp
                    <div>
                        ...
                        <select name="status" required class="w-full p-2.5 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white">
                            ...
                            <option value="ongoing" {{ old('status', $currentStatus) == 'ongoing' ? 'selected' : '' }}>
                                {{ app()->getLocale() == 'ar' ? 'مستمر (Ongoing)' : 'Ongoing (مستمر)' }}
                            </option>
                            <option value="in_progress" {{ old('status', $currentStatus) == 'in_progress' ? 'selected' : '' }}>
                                {{ app()->getLocale() == 'ar' ? 'قيد التنفيذ (In Progress)' : 'In Progress (قيد التنفيذ)' }}
                            </option>
                            <option value="planned" {{ old('status', $currentStatus) == 'planned' ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <input type="text" name="amount" value="{{ old('amount', $activity->amount) }}" placeholder="e.g. 2,000 USD" class="w-full p-2.5 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white">
                    ...
                    <div>
                        ...
                        <input type="text" name="icon" value="{{ old('icon', $activity->icon) }}" placeholder="e.g. fas fa-tint" class="w-full p-2.5 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white">
                ...
                <button type="submit" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3.5 px-6 rounded-xl shadow-md hover:shadow-lg transition flex justify-center items-center gap-2">
                    ...
                    <span>{{ app()->getLocale() == 'ar' ? 'تحديث النشاط والعودة' : 'Update Activity' }}</span>
                </button>
            </div>
        </div>
    </form>
