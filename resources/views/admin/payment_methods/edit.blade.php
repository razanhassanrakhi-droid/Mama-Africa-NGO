...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="mb-6 flex justify-between items-center">
        <h2 class="text-2xl font-bold text-slate-800">{{ app()->getLocale() == 'ar' ? 'تعديل طريقة الدفع:' : 'Edit Payment Method:' }} <span class="text-blue-600">{{ $paymentMethod->name }}</span></h2>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <form action="{{ route('admin.payment_methods.update', $paymentMethod->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="px-6 py-5 border-b border-slate-200 bg-slate-50/50">
                ...
                <p class="text-sm text-slate-500 mt-1">{{ app()->getLocale() == 'ar' ? 'تحديث المعلومات الخاصة بطريقة الدفع.' : 'Update the information for this payment method.' }}</p>
            </div>
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        ...
                        <input type="text" id="name_ar" name="name[ar]" value="{{ $paymentMethod->getTranslation('name', 'ar') }}" required
                    ...
                    <div>
                        ...
                        <input type="text" id="name_en" name="name[en]" value="{{ $paymentMethod->getTranslation('name', 'en') }}" required
                            class="w-full rounded-lg border-slate-300 border px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-shadow">
                    </div>
                    ...
                    <div>
                        ...
                        <select id="type" name="type" required
                            ...
                            <option value="bank" {{ $paymentMethod->type == 'bank' ? 'selected' : '' }}>{{ app()->getLocale() == 'ar' ? 'تحويل بنكي' : 'Bank Transfer' }}</option>
                            <option value="mobile_money" {{ $paymentMethod->type == 'mobile_money' ? 'selected' : '' }}>{{ app()->getLocale() == 'ar' ? 'موبايل موني' : 'Mobile Money' }}</option>
                            <option value="online" {{ $paymentMethod->type == 'online' ? 'selected' : '' }}>{{ app()->getLocale() == 'ar' ? 'بوابة دفع إلكتروني' : 'Online Gateway' }}</option>
                            <option value="crypto" {{ $paymentMethod->type == 'crypto' ? 'selected' : '' }}>{{ app()->getLocale() == 'ar' ? 'عملات رقمية' : 'Cryptocurrency' }}</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" id="icon" name="icon" value="{{ $paymentMethod->icon }}" placeholder="fas fa-university"
                    ...
                    <div class="md:col-span-2">
                        <label for="logo" class="block text-sm font-medium text-slate-700 mb-2">{{ app()->getLocale() == 'ar' ? 'تغيير اللوجو / الصورة (اختياري)' : 'Change Logo / Image (optional)' }}</label>
                        <div class="h-40 border-2 border-dashed border-blue-200 rounded-xl flex flex-col justify-center items-center bg-slate-50 hover:bg-white hover:border-blue-400 transition-all cursor-pointer group relative overflow-hidden">
                            ...
                            <div id="logoPlaceholder" class="w-full h-full flex flex-col items-center justify-center p-4 text-center {{ $paymentMethod->logo ? 'hidden' : '' }}">
                            ...
                            <img id="logoPreview" src="{{ $paymentMethod->logo ? url('/media/' . $paymentMethod->logo) : '' }}" class="absolute inset-0 w-full h-full object-contain p-4 {{ $paymentMethod->logo ? '' : 'hidden' }}" />
                            
                            <button id="removeLogoBtn" type="button" onclick="resetImageUpload('payment_logo', 'logoPreview', 'logoPlaceholder', 'removeLogoBtn', 'hidden_payment_logo')" class="absolute top-2 right-2 bg-rose-500 text-white w-7 h-7 rounded-full flex items-center justify-center hover:bg-rose-600 {{ $paymentMethod->logo ? '' : 'hidden' }} z-20 shadow-md transition-colors">
                    ...
                    <div>
                        ...
                        <textarea id="description_ar" name="description[ar]" rows="2"
                            class="w-full rounded-lg border-slate-300 border px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-shadow">{{ $paymentMethod->getTranslation('description', 'ar') }}</textarea>
                    </div>
                    <div>
                        ...
                        <textarea id="description_en" name="description[en]" rows="2"
                            class="w-full rounded-lg border-slate-300 border px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-shadow">{{ $paymentMethod->getTranslation('description', 'en') }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea id="instructions_ar" name="instructions[ar]" rows="4"
                            class="w-full rounded-lg border-slate-300 border px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-shadow">{{ $paymentMethod->getTranslation('instructions', 'ar') }}</textarea>
                    </div>
                    <div>
                        ...
                        <textarea id="instructions_en" name="instructions[en]" rows="4"
                            class="w-full rounded-lg border-slate-300 border px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-shadow">{{ $paymentMethod->getTranslation('instructions', 'en') }}</textarea>
                    </div>
                </div>

                <div class="border-t border-slate-200 pt-5 pb-2">
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" class="sr-only peer" {{ $paymentMethod->is_active ? 'checked' : '' }}>
            ...
            <div class="px-6 py-4 bg-slate-50 border-t border-slate-200 flex justify-end gap-3">
                ...
                <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-6 rounded-lg shadow-sm transition-colors flex items-center gap-2 text-sm">
                    ...
                    {{ app()->getLocale() == 'ar' ? 'تحديث طريقة الدفع' : 'Update Payment Method' }}
                </button>
            </div>
        </form>
    </div>
