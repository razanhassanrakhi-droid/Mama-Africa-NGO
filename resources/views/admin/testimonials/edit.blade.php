...
@section('title', __('massage.form_edit_testimonial') ?? 'Edit Testimonial')
@section('header', __('massage.form_edit_testimonial') ?? 'Edit Testimonial')
...
<div class="bg-white p-6 md:p-8 rounded-xl shadow-sm border border-gray-200 focus-within:ring-2 focus-within:ring-blue-100 transition-all max-w-5xl mx-auto">
    <div class="flex items-center justify-between mb-6 pb-4 border-b border-gray-100">
        <h3 class="text-xl font-bold text-gray-800">{{ __('massage.form_edit_testimonial') }}</h3>
                ...
                <div class="bg-gray-50 p-4 rounded-lg border border-gray-100">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="name[ar]" required value="{{ old('name.ar', $testimonial->getTranslation('name','ar')) }}" class="w-full p-2.5 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white" dir="rtl">
                        </div>
                        <div>
                            <label class="block text-sm font-semibold mb-1 text-gray-600">{{ __('massage.form_name_en') }} <span class="text-red-500">*</span></label>
                            <input type="text" name="name[en]" required value="{{ old('name.en', $testimonial->getTranslation('name','en')) }}" class="w-full p-2.5 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white" dir="ltr">
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-gray-50 p-4 rounded-lg border border-gray-100">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <textarea name="comment[ar]" required rows="4" class="w-full p-2.5 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white" dir="rtl">{{ old('comment.ar', $testimonial->getTranslation('comment','ar')) }}</textarea>
                        </div>
                        <div>
                            <label class="block text-sm font-semibold mb-1 text-gray-600">{{ __('massage.form_comment_en') }} <span class="text-red-500">*</span></label>
                            <textarea name="comment[en]" required rows="4" class="w-full p-2.5 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white" dir="ltr">{{ old('comment.en', $testimonial->getTranslation('comment','en')) }}</textarea>
                        </div>
                    </div>
                    <!-- Video Link -->
                <div class="bg-gray-50 p-4 rounded-lg border border-gray-100">
                    ...
                    <div>
                        <input type="url" name="video_link" value="{{ old('video_link', $testimonial->video_link) }}" placeholder="https://youtube.com/..." class="w-full p-2.5 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white" dir="ltr">
                    </div>
                </div>
            </div>
            </div>
                ...
                <div class="sticky top-6">
                    ...
                    <div class="bg-gray-50 p-4 rounded-lg border border-gray-100">
                        ...
                        <div class="h-64 border-2 border-dashed border-blue-300 rounded-xl flex flex-col justify-center items-center bg-white hover:bg-blue-50 transition cursor-pointer group relative overflow-hidden">
                            ...
                            <div id="placeholder_testimonial_image" class="w-full h-full flex flex-col items-center justify-center p-4 text-center {{ $testimonial->image ? 'hidden' : '' }}">
                            ...
                            <img id="preview_testimonial_image" src="{{ $testimonial->image ? url('/media/'.$testimonial->image) : '' }}" class="absolute inset-0 w-full h-full object-cover {{ $testimonial->image ? '' : 'hidden' }}" />
                            
                            <button id="remove_testimonial_image" type="button" onclick="resetImageUpload('testimonial_image', 'preview_testimonial_image', 'placeholder_testimonial_image', 'remove_testimonial_image', 'hidden_testimonial_image')" class="absolute top-2 right-2 bg-red-500 text-white w-8 h-8 rounded-full flex items-center justify-center hover:bg-red-600 {{ $testimonial->image ? '' : 'hidden' }} z-20 shadow-md transition-colors" title="Remove Photo">
                    ...
                    <div class="mt-8">
                        <button type="submit" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3.5 px-6 rounded-xl shadow-md hover:shadow-lg transition flex justify-center items-center gap-2">
                            ...
                            <span>{{ __('massage.form_update_testimonial') }}</span>
                        </button>
                    </div>
                </div>
            </div>
