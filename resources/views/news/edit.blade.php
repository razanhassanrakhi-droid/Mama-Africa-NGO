...
<main class="container mx-auto px-4 pb-10 flex-grow">
    ...
    <div class="bg-white p-6 rounded-2xl shadow-xl border border-gray-100 w-full max-w-6xl mx-auto">
        ...
        <h2 class="text-2xl font-bold mb-6 text-center">تعديل المشروع</h2>
                ...
                <div class="lg:col-span-3 space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <textarea name="details[ar]" rows="4"
                                class="w-full p-2.5 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all">{{ old('details.ar', $news->getTranslation('details','ar')) }}</textarea>
                        </div>

                        <!-- تفاصيل المشروع بالانجليزي -->
                        <div>
                            <label class="block text-sm font-semibold mb-1 text-gray-600">  {{ __('massage.project_details_en') }}</label>
                            <textarea name="details[en]" rows="4"
                                class="w-full p-2.5 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all">{{ old('details.en', $news->getTranslation('details','en')) }}</textarea>
                        </div>
    <div class="mb-4">
        <label class="block text-sm font-semibold mb-1 text-gray-600">   {{ __('massage.challange_ar') }}</label>
                            <textarea name="challange[ar]" rows="4"
                                class="w-full p-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500">{{ old('challange.ar', $news->getTranslation('challange', 'ar')) }}</textarea>
                            ...
                            <textarea name="challange[en]" rows="4"
                                class="w-full p-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500">{{ old('challange.en', $news->getTranslation('challange', 'en')) }}</textarea>
                        </div>
                        ...
                        <div class="col-span-2">
                            <label class="block text-gray-700 font-bold mb-1 text-lg">   {{ __('massage.project_category') }} </label>
                            <select name="project_id" class="w-full p-2.5 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all">
                                <option value="">اختر المشروع</option>
                                @foreach ($projects as $project)
                                    <option value="{{ $project->id }}" {{ $news->project_id == $project->id ? 'selected' : '' }}>
                                        {{ $project->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-span-2">
                            ...
                            <input type="number" name="pay" 
                                value="{{ old('pay', $news->pay) }}"
                                class="w-full p-2.5 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-pink-500 focus:border-pink-500 outline-none transition-all shadow-sm">
                        </div>
                ...
                <div class="lg:col-span-1">
                    <label class="block text-sm font-semibold mb-2 text-gray-600 text-center">{{ __('project_image') }} </label>
                    <div class="h-full min-h-[200px] border-2 border-dashed border-gray-300 rounded-xl flex flex-col justify-center items-center bg-gray-50 hover:bg-gray-100 transition cursor-pointer group relative overflow-hidden">
                        ...
                        <div id="imagePreviewContainer" class="w-full h-full flex flex-col items-center justify-center p-2">
                            @if($news->image)
                                <img src="{{ url('/media/' . $news->image) }}" class="w-full h-full object-cover rounded-xl">
                            @else
            ...
            <div class="flex-1"> 
                ...
                <input type="url" name="youtube_link" value="{{ old('youtube_link', $news->youtube_link) }}" class="w-full p-2.5 rounded-lg border border-gray-300"> 
            </div>
