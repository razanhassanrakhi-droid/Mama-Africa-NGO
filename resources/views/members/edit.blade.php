...
<main class="container mx-auto px-4 pb-10 flex-grow">
    ...
    <div class="bg-white p-6 rounded-2xl shadow-xl border border-gray-100 w-full max-w-6xl mx-auto">
        ...
        <h2 class="text-2xl font-bold mb-6 text-center"> {{ __('massage.members_management') }}</h2>
...
<input type="text" name="name[ar]"
       value="{{ old('name.ar', $member->getTranslation('name','ar')) }}"
...
<input type="text" name="name[en]"
       value="{{ old('name.en', $member->getTranslation('name','en')) }}"
       class="w-full p-2.5 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500">
                        </div>
...
<input type="text" name="position[ar]"
       value="{{ old('position.ar', $member->getTranslation('position','ar')) }}"
       class="w-full p-2.5 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500">
...
<input type="text" name="position[en]"
       value="{{ old('position.en', $member->getTranslation('position','en')) }}"
       class="w-full p-2.5 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500">
                        </div>
                         <div class="col-span-2">
<label class="block text-sm font-semibold mb-1 text-gray-600">
...
<textarea name="role[ar]"
          class="w-full p-2.5 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('role.ar', $member->getTranslation('role','ar')) }}</textarea>

<label class="block text-sm font-semibold mb-1 text-gray-600 mt-3">
 {{ __('massage.role_en') }}
</label>
<textarea name="role[en]"
          class="w-full p-2.5 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('role.en', $member->getTranslation('role','en')) }}</textarea>
                        </div>
                ...
                <div class="lg:col-span-1">
                    ...
                    <div class="h-full min-h-[200px] border-2 border-dashed border-gray-300 rounded-xl flex flex-col justify-center items-center bg-gray-50 hover:bg-gray-100 transition cursor-pointer group relative overflow-hidden">
                        ...
                        <div id="imagePreviewContainer" class="w-full h-full flex flex-col items-center justify-center p-2">
                            @if($member->image)
                                <img src="{{ url('/media/' . $member->image) }}" class="w-full h-full object-cover rounded-xl">
                            @else
                                ...
                                <span class="text-sm text-gray-500 font-medium group-hover:text-blue-600 text-center">{{ __('massage.upload_image') }}</span>
                            @endif
                        </div>
...
function removeImage(event) {
    ...
    previewContainer.innerHTML = `
        @if( $member->image)
            <img src="{{ url('/media/' .  $member->image) }}" class="w-full h-full object-cover rounded-xl" alt="current">
        @else
